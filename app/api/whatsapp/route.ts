...
const DONE_PHRASES = [
  ...
  'залили', 'уложили', 'забетонировали',
  'заасфальтировали', 'заштукатурили',
  'покрасили', 'поклеили', 'зашпаклевали',
  'подписали акт', 'акт подписан', 'акт принят',
  'принято', 'сдано', 'сдана', 'сданы',
...
const RED_PHRASES = [
  ...
  'недоделали', 'недоделано',
  'не докончили', 'не довели до конца',
  'не заделали', 'не зашили', 'не обшили',
  'не затянули', 'потолок не готов', 'стены не готовы', 'пол не готов',
  'работы остановлены', 'срок сорван', 'сроки сорваны',
  'сломался', 'сломалась', 'сломались', 'поломка', 'авария',
  'не работает', 'насос сломался',
  // Срывы
  'не залили', 'не уложили', 'не забетонировали',
  'не заасфальтировали', 'не закрыли', 'не сдали',
  'не приняли', 'не привезли материал',
  'не зашли на объект', 'не вышла бригада',
  'бригада не пришла', 'никого нет на объекте',
  // Дефекты
  'трещина', 'трещины', 'скол', 'сколы',
  'расслоение', 'отслоение', 'обрушение',
  'обвал', 'провал', 'просадка', 'деформация',
  'протечка', 'течь', 'затопление', 'подтопление',
  'плесень', 'грибок', 'коррозия', 'ржавчина',
  'брак', 'переделка', 'несоответствие',
  'неправильно установили', 'неверно смонтировали',
  'ошибка монтажа', 'нарушение технологии',
...
const YELLOW_PHRASES = [
  ...
  'не успеваем', 'под угрозой', 'может сорваться',
  'выбиваемся из графика', 'отстаём', 'отстаем',
  'отклонение по срокам', 'под вопросом',
  'нет техники', 'техника сломалась',
  'мало рабочих', 'не хватает людей',
  'из-за дождя', 'из-за мороза', 'из-за ветра',
  'из-за погоды', 'плохая погода',
  'не можем работать', 'приостановили',
  'ждём погоды', 'ждем погоды',
  'нет доступа', 'не пускают',
...
const STAGE_RULES: { stage: string; parts: string[] }[] = [
  ...
  { stage: 'монолит', parts: ['монолит', 'армир', 'опалуб', 'перекрыт', 'ригел', 'колонн', 'заливк', 'колонны', 'пилоны', 'ригели', 'балки', 'прогоны', 'вязка арматуры', 'съём опалубки', 'съем опалубки', 'заливка бетона', 'кладка', 'кирпичная кладка', 'газобетон', 'пеноблок', 'керамзитоблок'] },
  { stage: 'кровля', parts: ['кровл', 'крыша', 'стропил', 'мембран', 'черепиц', 'профлист', 'обрешётка', 'обрешетка', 'контробрешётка', 'металлочерепица', 'профнастил', 'мягкая кровля', 'рулонная кровля', 'битумная черепица', 'фальцевая кровля', 'водосток', 'водосточная', 'утепление кровли', 'пароизоляция', 'конёк', 'конек', 'парапет', 'примыкание', 'карниз'] },
  { stage: 'инженерия', parts: ['электрик', 'электромонтаж', 'кабель', 'кабел', 'щитовая', 'щит', 'освещени', 'розетк', 'розет', 'сантехник', 'сантех', 'водопровод', 'водоснаб', 'канализац', 'канализ', 'отоплени', 'отоплен', 'радиатор', 'тёплый пол', 'теплый пол', 'котельн', 'вентиляц', 'воздуховод', 'кондиционер', 'лифт', 'газ', 'газификац', 'пожарн', 'слаботочк', 'видеонаблюдени', 'интернет кабель', 'труб'] },
  { stage: 'окна', parts: ['окн', 'стеклопакет', 'подоконник', 'откос', 'дверн', 'входная группа', 'витраж', 'перегородк'] },
  { stage: 'отделка', parts: ['штукатур', 'стяжк', 'плитк', 'шпаклев', 'покраск', 'потол', 'обои', 'ламинат', 'гипсокартон'] },
  { stage: 'фасад', parts: ['фасад', 'утепл', 'облицовк', 'мокрый фасад', 'утепление фасада', 'минвата', 'вентфасад', 'декоративная штукатурка', 'облицовка', 'кирпич облицовочный', 'клинкер', 'керамогранит', 'навесной фасад', 'кассеты', 'окраска фасада', 'цоколь', 'отлив', 'оконный отлив'] },
      ...
      if (!companyId) {
        ...
        try {
          await supabase.from('unknown_whatsapp_messages').insert({
            id_instance: String(idInstance || ''),
            raw_body: JSON.stringify(body).slice(0, 2000),
            sender_phone: senderPhone || '',
            message_text: title || '',
            created_at: new Date().toISOString(),
          })
        } catch (e) {
          console.log('Failed to save unrouted message:', e)
        }
